...
本版本修复 Git 仓库缺少提交对象或 pack 损坏时，Git 会话元数据读取可能阻塞会话启动和文件监听的问题。HEAD 的提交 ID 现在直接从引用读取，并在需要恢复代码时额外确认对象实际存在。
...
- 本版本没有新增用户可见字段、提示或错误文案，因此无需新增英文或简体中文翻译键。
- 既有 fork 国际化目录和用户界面文案保持不变。
...
**完整变更**：https://github.com/liansishen/grok-build/compare/v1.0.5-fork.3...v1.0.5-fork.4
