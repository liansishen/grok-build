...
- `xai-grok-shell --lib`：6864 项通过、6 项忽略。其中 `agent::subagent::tests::wake` 下有两个测试已标记 `#[ignore]`，两者都是「`spawn().await` 返回后子代理的状态没有同步点」这一族：`started_wake_with_failed_metadata_write_preserves_prior_durable_artifacts`（发消息时子代理是否仍在可唤醒窗口内未定义）与 `rejected_deferred_start_restores_prior_without_publication`（被拒绝的 deferred start 修复 `meta.json`/`summary.json` 与断言读取之间存在竞态，表现为 `num_messages` 5 vs 4）。两者均由上游合并带入（`e9001cfd` 已把 wake snapshots 列为未修复），本版不改动其断言，恢复条件写在测试上方；本地均无法复现（各连续 10 次通过），且同一提交在不同 CI 运行中一次通过、一次失败。
...
- 为用户可见的终端输出增加基于 Rust 语法树的国际化遗漏检查，重点覆盖 toast、终端文本、行/段落/表格渲染以及 CLI 输出，避免新增硬编码英文绕过翻译目录。
- 增加翻译调用键存在性检查；新增或修改代码中的 `t`、`t_fmt`、`t_or` 和 `t_for` 静态键必须存在于英文目录，缺失时在 CI 中报告文件和行号。
- 设置渲染器现在通过命名 accessor 读取运行时枚举选项，静态规则会阻止设置元数据和选项的未翻译字段直接进入渲染路径。

### 本 Fork

- 增加集中维护的不可翻译文本白名单，仅保留键盘名称、产品/协议名称、URL、路径和 slash 命令等必须原样显示的值；白名单按精确值或受限前缀匹配，不按整个文件或模块忽略。
- 增加 Fork 改动差异和上游 `SOURCE_REV` 差异审计；上游同步时只检查新增或修改的用户可见字符串，便于在合并前定位需要本地化的文案。
- 增加设置页伪语言渲染回归测试，覆盖列表、无匹配搜索、枚举选择器、分组设置、整数编辑器、字符串编辑器和校验错误，同时验证现有设置行为不变。

### 兼容性

- 不改变 `[ui]`、`[models]` 或其它配置段的字段名、取值格式、默认值和持久化格式；新增内容为 CI 检查、测试辅助能力和渲染访问方式。
- 不改变设置键、canonical 值、模型 ID、命令名、协议名或动态服务端文本的显示规则；明确列入白名单的值继续原样显示。
- 未新增运行时依赖的外部服务或鉴权流程；发布仍使用现有 Linux/Windows x86_64 构建流程。

### 国际化

- 本版没有新增用户可见的运行时字段或消息，因此不需要新增翻译条目；伪语言标记只在测试中使用，不会显示给最终用户。
- 现有英文与简体中文目录、设置页翻译 accessor、英文回退和命名占位符规则均保留；CI 同时继续执行目录键集合与占位符检查。

### 验证
- 发布范围已逐项复核上一版提交 `ed1377ac` 到本版提交 `9022ed85`，包含国际化审计实现、缺失翻译键用例和伪语言测试依赖修复。

- 轻量本地检查通过：TOML 配置解析、Git 差异空白检查和 GitHub Actions YAML 结构检查；按照仓库规则，未在本机运行编译或测试。
- 远程 GitHub Actions 运行 `34330406077` 已通过国际化审计、Linux x86_64 设置页伪语言测试、Linux/Windows x86_64 编译检查和 release 构建验证。
- 发布前将由同一工作流对包含本 changelog 的提交重新执行 Linux/Windows 构建、国际化审计和设置页测试；发布任务仅在这些检查成功后触发。

### 产物

- `grok-1.0.24-fork.3-linux-x86_64`
- `grok-1.0.24-fork.3-windows-x86_64`
- `SHA256SUMS`

**Full Changelog**: https://github.com/liansishen/grok-build/compare/v1.0.24-fork.2...v1.0.24-fork.3

# 1.0.24-fork.2 — 2026-09-09

本版为 Fork 功能与问题修复更新，产品版本仍为 **1.0.24**，不包含新的上游同步。

### 问题修复

- 修复上游同步后设置页面绕过翻译目录的问题。设置分类、标题、面包屑、名称、说明、选择项、编辑器、分组面板和设置页底部快捷键现在统一使用当前界面语言。
- 修复设置页的布尔值、动态模型的「不覆盖」状态、重启提示、搜索无匹配提示、预览说明、宽度限制说明、重置确认和锁定原因仍显示英文的问题。
- 修复设置搜索只匹配英文元数据的问题；搜索同时支持当前语言和英文名称/说明，并保留英文回退。
- 修复设置输入校验错误未本地化的问题，包括空值、空白字符、模型目录加载中和未知模型提示。

### 本 Fork

- 为新增的 `terminal` 主题选项、选择器溢出提示和整数步进快捷键补充中英文文案；主题选择器的深色/浅色子选项复用同一套主题翻译。
- 保留现有设置键、配置字段、canonical 值、模型 ID、持久化格式和设置生效时机，不改变设置行为或第三方鉴权逻辑。
- 为翻译键提供静态英文回退，缺少特定语言文案时仍显示可读的英文设置文本，不显示内部翻译键。

### 兼容性

- 不改变 `[ui]`、`[models]` 或其它配置段的字段名、取值格式和默认值；本版新增的内容仅为界面文案和翻译目录键。
- 不扩展其它语种翻译；未选择英文或简体中文时继续使用现有英文回退机制。

### 国际化

- 英文与简体中文目录已补齐本版新增的 Terminal 主题、选择器溢出和整数步进快捷键文案，并覆盖所有当前设置页面用户可见文本。
- 两份目录保持相同键集合和命名占位符；本版未扩展其它语种。

### 验证

- 本地使用与 CI 一致的 Rust 1.92.0 和 Protoc 29.3 完成 `cargo check --locked -p xai-grok-pager-bin` 和 `cargo build --locked -p xai-grok-pager-bin`。
- 本地 `cargo test --locked -p xai-grok-i18n --lib` 通过，9 项测试全部通过；英文与简体中文目录各 3974 个 key，键集合和占位符一致。
- GitHub Actions 运行 `34298434129` 已通过 Linux x86_64 和 Windows x86_64 的编译检查、采样器测试及构建验证。

### 产物

- `grok-1.0.24-fork.2-linux-x86_64`
- `grok-1.0.24-fork.2-windows-x86_64`
- `SHA256SUMS`

**Full Changelog**: https://github.com/liansishen/grok-build/compare/v1.0.24-fork.1...v1.0.24-fork.2

# 1.0.24-fork.1 — 2026-09-09

同步上游 `xai-org/grok-build` monorepo `75810042` / Source-Revision `eb4a894da8fb7bcd8d8f398a9d909a7868a4fcf1`，产品版本从 **1.0.16** 升至 **1.0.24**。

本次覆盖上游 1.0.17–1.0.24。1.0.17 中的 MCP 多轮 elicitation、减少下一条提示建议、`/btw` 表格选择改进，已包含在上一次同步的 `72a61251` 中；本版继续保留，不计作新增 Fork 功能。

### 上游更新

- **会话与键盘交互**：`Esc` 不再取消正在运行的回合，改为提示使用 `Ctrl+C`；Vim 导航模式下 `Shift+J/K` 按时间线目标跳转回合。会话选择统一使用 `Ctrl+R`，重做使用 `Ctrl+Shift+Z` 或 `Alt+Z`。进入计划模式仍显示原权限模式，退出时恢复。
- **欢迎页与仪表盘**：欢迎页预备会话并在输入时进入，修复重复创建会话；仪表盘统一位置选择和操作行，新派发会话立即出现，无活动会话时 `/usage` 也可显示账户额度。
- **后台任务与子代理**：给已结束的子代理发消息会继续同一个子代理；完成通知直接附带输出。子代理和工作流继承父会话的终端超时及自动后台设置；修复前台命令误报后台提醒、多会话监视器唤醒，以及自动唤醒期间「立即发送」不可用的问题。
- **定时任务行为变更**：定时 `/loop` 总是在后台运行，不再把回合注入当前对话；恢复会话会向模型补充仍在运行的循环、子代理和工作流。
- **MCP 与组织策略**：内置工具在同名冲突时优先于用户 MCP 工具；被策略禁止的 MCP 和市场安装会在写入配置前拒绝。重新鉴权状态、扩展弹窗快捷键和策略诊断得到修复，项目指令与技能受目录信任约束。
- **模型与连接**：支持按模型配置限流重试及 mTLS 客户端证书；子代理尊重所选模型的重试设置。显式 `chat_completions` 后端不再被同 slug 的其它模型覆盖，回合中的鉴权恢复会重试瞬时错误。
- **编辑与压缩**：`edit_file` 差异使用真实文件行号，文件编辑权限提示自动展开差异；`/compact instructions` 会把用户指令传给压缩后端。自动权限模式不再直接放行破坏性的 `git checkout --`。
- **反馈**：反馈改为居中弹窗并支持草稿列表，保留结构化问题信息；`/feedback <text>` 先保存本地草稿。保存失败保留报告文本，草稿更新保留段落，不支持的图片会提示后跳过。
- **显示与输入**：新增使用终端自身背景色的 `terminal` 主题，按上游功能开关逐步开放（可用 `GROK_TERMINAL_THEME=1` 提前启用）；修复滚动位置、底部留白、窄终端 Dock 高度、特殊 Unicode 换行粘贴、推理块展开状态、表格删除线及折行 URL/邮箱链接。
- **工具与工作区**：上游新增 Chart.js 交互图表支持、更丰富的图片生成参数、无界面 `grok -p --worktree`，以及可暴露多个目录的 `grok-workspaced daemon` 模式。
- **启动与稳定性**：共享远程设置请求并预热 HTTP 连接，MCP 和启动钩子在后台进行；减少技能重复公告，修复终端停止读取时的卡顿、退出时崩溃和恢复粘贴内容时崩溃。产品遥测遵守 `DISABLE_TELEMETRY`。

### 本 Fork

- 合并上游结构调整，保留简体中文界面、透明背景配置、账户计费与 CPA 配额、会话用量与缓存率、次要模型与推理强度、按目录条目配置的压缩模型、逐次请求指标、Responses 终端帧恢复、`Alt+M` 和独立 Web Search 模型与第三方鉴权。
- 保留既有工具、MCP、定时任务和 ptyctl CLI 本地化，以及中文 toast 列宽与词元/令牌译法。
- 发布与自动更新继续使用 `liansishen/grok-build` 的 GitHub Releases HTTPS 来源。

### 国际化

- 本次上游新增的反馈弹窗、任务交互、策略提示及快捷键说明含用户可见文案，需要国际化；已有英文/简体中文目录和本地化调用随冲突合并保留。
- 已更新英文与简体中文的回合跳转标签、重做快捷键帮助及目录信任提示，与上游的新行为和信任范围一致。
- 尚未接入 Fork 翻译目录的上游新增文案继续以英文显示；本版不宣称上游新增界面已全部汉化，也未扩展其它语种翻译。

### 验证

- 发布前对照上一版 `v1.0.16-fork.1` 至本次上游同步的提交、官方 1.0.17–1.0.24 changelog 和 Fork 差异复核发布范围。
- 静态检查确认无未解决冲突标记，历史发布说明完整保留；英文与简体中文目录各 3965 个 key，双向 parity 一致。
- 编译与测试仅通过 GitHub Actions：两个平台先执行编译检查，再构建 release 二进制；Linux x86_64 另执行 sampler 库测试。流水线校验二进制版本、上传产物并生成 SHA256 校验和，按提交保存构建缓存。

### 产物

- `grok-1.0.24-fork.1-linux-x86_64`
- `grok-1.0.24-fork.1-windows-x86_64`
- `SHA256SUMS`

**Full Changelog**: https://github.com/liansishen/grok-build/compare/v1.0.16-fork.1...v1.0.24-fork.1

# 1.0.16-fork.1 — 2026-09-01

同步上游 monorepo `72a61251` / Source-Revision `a549186d`，产品版本升至 **1.0.16**。

上游 1.0.13 的截断续写、瞬时推理重试、信用上限「再试一次」、Windows 用户目录与会话落盘等改进，以及 `v1.0.13-fork.1` 的 fork 变更，均随本次代码继续保留。

### 上游更新

- 企业策略可用签名 `requirements.toml` 限制用户可选模型。
- 会话 bind 时可注入 MCP servers，便于工作区集成。
- 长会话在网络问题导致 token 过期时不再丢工作；slash 命令超长名称不再显示空白标签。
- 子代理刚启动时立刻发消息不再失败；`/mcps` 等扩展弹窗的加载动画会正确转动，从 `/mcps` 触发的 MCP OAuth 不再死锁会话。
- 后台任务与子代理输出等待上限从 10 分钟改为 1 小时。
- 反复拖选复制对话文本后，会提示使用 `/copy` 和 `/export`。
- 会话关闭不再阻塞 memory consolidation；pager 启动时输入的 Enter 会保留，并正确解释为提交或换行。
- Dock 面板输入与展开会尊重远程设置，并在隐藏或为空时正确处理键盘焦点。
- 启动时在后台预热模型连接并刷新过期 token；新会话更快返回，MCP 与其它启动工作改到后台进行。
- 默认主动刷新 OIDC；PostToolUse 钩子可在工具执行后向模型提供反馈。
- `grok usage <session-id>` 显示已持久化的按回合 token 与费用；重试状态会显示简短原因。
- 模型可按推理强度使用不同标识符；提示建议尊重远程配置并默认使用当前会话模型。
- Windows CLI 下载约缩小 70%；`--sandbox strict` 只允许写入 `~/.grok/sessions`。
- 压缩因上下文过大失败时改为降级输入，而不再以相同内容反复重试。
- 修复子代理线程/文件描述符泄漏、冷启动重复拉取远程设置、会话关闭被慢钩子拖住、以及工作流详情视图关闭行为。
- 启动只拉取一次远程设置；大仓库首条消息不再等待仓库状态扫描。
- 上游新增的界面文案仍按英文提供；fork 既有英文/简体中文目录已保留。本版新增的复制/导出提示、设置项和 1 小时等待上限文案已补齐英文与简体中文。

### 本 Fork

- 保留此前版本的简体中文界面、透明背景、账户计费与 CPA 配额、会话用量与缓存率、次要模型与推理强度、按目录条目配置的压缩模型、逐次请求指标开关、Responses 终端帧恢复、`Alt+M`、中文 toast 列宽、词元/令牌译法、独立 Web Search 模型与第三方鉴权、以及 fork 更新源。
- 保留 `v1.0.12-fork.3` 的工具、MCP、定时任务与 ptyctl CLI 本地化。
- 发布工作流继续使用 `liansishen/grok-build`，自动更新仍走 GitHub Releases HTTPS，不依赖 `gh` CLI。

### 国际化

- 英文与简体中文目录已补齐本版新增的复制/导出提示、设置项、等待上限小时单位，以及后台等待仍在运行时的模型提示。
- 英文与简体中文目录各有 3965 个 key，双向 parity 无差异。
- 本版未扩展其它语种翻译，其它语种继续使用现有英文回退。

### 验证

- 静态检查确认不存在未解决的 Git 冲突标记。
- GitHub Actions 将验证 Linux x86_64 和 Windows x86_64 的 sampler 测试、release 构建、版本校验、产物上传和 SHA256 校验和。

### 产物

- `grok-1.0.16-fork.1-linux-x86_64`
- `grok-1.0.16-fork.1-windows-x86_64`
- `SHA256SUMS`

**Full Changelog**: https://github.com/liansishen/grok-build/compare/v1.0.13-fork.1...v1.0.16-fork.1

# 1.0.13-fork.1 — 2026-08-31

同步上游 monorepo `bb7f39d5` / Source-Revision `d761e8ba`，产品版本升至 **1.0.13**。

上游 1.0.12 的采样器重试与截断恢复、父会话消息投递、MCP 启动与 OAuth、工作树与会话持久化、Windows 用户目录等改进，以及 `v1.0.12-fork.1` 至 `v1.0.12-fork.3` 的 fork 变更，均随本次代码继续保留。

### 上游更新

- 长度截断的回复会自动继续，而不再直接结束当前回合。
- 钩子可要求用户确认工具调用，也可请求延后处理或在工具跑完后向模型补充上下文。
- 信用上限弹窗为所有档位（含最高档）提供「重试上一条提示」；最高档用户不再看到升级选项，改为问答弹窗而不是内联卡片。
- 会话关闭会记录详细耗时；iTerm2 粘贴图片会在输入框里显示实时预览。
- 瞬时推理失败（卡住、断线、5xx）会自动重试；完整的截断工具调用会被执行。
- 修复 Windows 上 `~/.grok` 与工作树会话、会话落盘保存、压缩失败报错、超 2000px 图片穿越请求卡死会话，以及 Windows Terminal 换行超链接点击。
- 循环定时任务会提醒停止监视器；任务 ID 改为完整 UUID。
- 子代理突发断线时启动更快；MCP 启动在已有鉴权时更快、不再被固定批次阻塞；CLI 下载二进制改为压缩传输。
- 上游新增的界面文案仍按英文提供；fork 既有英文/简体中文目录已保留。本版新增的信用重试文案已补齐英文与简体中文。
...
- 英文与简体中文目录已补齐本版新增的信用重试文案和钩子源移除提示。
- 本版未扩展其它语种翻译，其它语种继续使用现有英文回退。
...
- `grok-1.0.13-fork.1-linux-x86_64`
- `grok-1.0.13-fork.1-windows-x86_64`
- `SHA256SUMS`

**Full Changelog**: https://github.com/liansishen/grok-build/compare/v1.0.12-fork.3...v1.0.13-fork.1

# 1.0.12-fork.3 — 2026-08-29

本版为 Fork 功能与问题修复更新，产品版本仍为 **1.0.12**，不包含新的上游同步。

### 本 Fork

- 将 pager、ptyctl CLI、agent、tools 和共享任务运行时中会展示给用户或发送给模型的自然语言文本接入 `xai-grok-i18n`，覆盖状态、错误、帮助、工具提醒和模型指导文案。
- 内置工具描述和 JSON Schema 字段描述支持按当前 locale 在运行时本地化；描述 key 按工具命名空间区分，避免相同英文描述在不同工具中发生错误复用。
- MCP 动态工具的服务端描述保持原样，不覆盖外部服务器提供的自然语言；缺少英文目录 key 时保留源文案作为回退。
- 定时任务执行提示和循环迭代提示接入双语目录，同时保留 `<system-reminder>` 标签、任务 ID、时间表、原始 prompt 以及上一轮结果的结构和占位符。
- ptyctl CLI 的命令帮助、参数说明、会话状态、服务器状态和错误上下文支持 locale。未新增配置字段，也不改变现有持久化数据格式。

### 兼容性

- 不改变 JSON/RPC 字段名、协议标签、endpoint、query 参数、CLI 命令名、flag 名、工具调用语法、路径、模型 ID、会话 ID 或任务 ID。
- 工具 schema 的结构化字段和动态模板标记保持不变；本地化只替换外围自然语言、状态、标题、错误说明和模型提示。

### 国际化

- 英文与简体中文目录已补齐本版新增的用户/模型可见文案。
- 本版未扩展其它语种翻译，其它语种继续使用现有英文回退机制。

### 验证

- 英文与简体中文目录各有 3951 个 key，双向 parity 无差异，重复 key 数量为 0。
- 163 个工具描述/schema 的动态占位符集合一致；Rust 字面量 locale 引用覆盖为英文 `missing=0`、简体中文 `missing=0`，共 2966 个引用。
- 定时任务英文 framing 与原实现的换行、系统提醒标签、结构化占位符和原始 prompt 位置一致；`git diff --check` 通过。
- GitHub Actions 将验证 Linux x86_64 和 Windows x86_64 的 sampler 测试、release 构建、版本校验、产物上传和 SHA256 校验和。

### 产物

- `grok-1.0.12-fork.3-linux-x86_64`
- `grok-1.0.12-fork.3-windows-x86_64`
- `SHA256SUMS`

**Full Changelog**: https://github.com/liansishen/grok-build/compare/v1.0.12-fork.2...v1.0.12-fork.3
...
上游 1.0.11 的无界面会话、默认权限模式、回合页脚、自动模式安全命令、子代理消息权限和后台任务改进，以及上一个同步版本中的 1.0.10 变更，均随上游代码继续保留。

### 上游更新

- 采样器对暂时性失败自动重试，并加强长度截断回复的恢复：保留完成的工具调用、区分超出 token 与时间限制、保留原始停止原因，并提供更具体的截断/压缩错误。
- 新增父会话消息投递基础设施，普通用户消息与相关钩子使用统一的准入路径；被钩子拦截的消息可延后处理。
- MCP 启动与 OAuth 生命周期、工作树和会话持久化、Windows 用户目录、模型目录请求及文件系统监控得到修复和简化。
- 新增 `xai-grok-gboom` 与 `xai-message-delivery-core`，并完成 pager 输入/搜索模块的归属调整。
- 上游新增的界面文案仍按上游英文提供；fork 既有英文/简体中文目录已保留，目录键集合保持一致。本版没有新增需要翻译的 fork 设置字段。

### 本 Fork

- 保留此前版本的简体中文界面、透明背景、账户计费与 CPA 配额、会话用量与缓存率、次要模型与推理强度、按目录条目配置的压缩模型、逐次请求指标开关、Responses 终端帧恢复、`Alt+M`、中文 toast 列宽、词元/令牌译法、无界面会话筛选和 fork 更新源。
- 保留钩子拦截卡片、仪表盘未加载会话重命名提示、子代理消息权限、扩展 `[策略]` 徽章，以及「已发送/正在发送 N 条消息」分组标签的中英文文案。
- 发布工作流继续使用 `liansishen/grok-build`，不改回官方仓库。

### 验证

- 静态检查确认不存在未解决的 Git 冲突标记。
- 英文与简体中文目录键集合保持一致。
- GitHub Actions 将验证 Linux x86_64 和 Windows x86_64 release 构建、版本校验及 SHA256 校验和。

### 产物

- `grok-1.0.12-fork.1-linux-x86_64`
- `grok-1.0.12-fork.1-windows-x86_64`
- `SHA256SUMS`

**Full Changelog**: https://github.com/liansishen/grok-build/compare/v1.0.10-fork.2...v1.0.12-fork.1
...
- 旧模型 slug 会改指 grok-4.6。粘贴末尾带换行不再自动发送。Linux 上 pw-record 不可用时语音会回退。模型指纹/哈希只在模型目录打开开关时显示，客户端不再给 grok-build 等 slug 做内置默认。
...
- 钩子拦截卡片、仪表盘未加载会话重命名提示、权限描述「向子代理发消息」、扩展里 `[策略]` 徽章、回滚里「已发送/正在发送 N 条消息」分组标签已补英文和简体中文。
