...
fn run_local_for_bridge_test_large_stack<F, Fut, T>(body: F) -> T
where
    ...
    T: Send + 'static,
{
    std::thread::Builder::new()
        ...
        .spawn(move || run_local_for_bridge_test(body))
        .expect("large-stack test thread must spawn")
        .join()
        .expect("large-stack test thread must not panic")
}
#[test]
...
fn new_session_registers_root_identity() {
    run_local_for_bridge_test_large_stack(|| async {
        let agent = build_minimal_agent_for_tests();
...
fn adopting_attach_waits_for_the_installed_actors_stamp() {
    run_local_for_bridge_test_large_stack(|| adopting_attach_fails_with_the_stamp(async |_, _| {}));
}
