...
async fn save_config_locked(
    ...
    let root: TomlValue = match tokio::fs::read_to_string(dest.as_path()).await {
    ...
    save_config_root_locked(guard, slot, dest, config, root).await
}

/// Merge an already-loaded root, serialize it, and publish it through `guard`.
/// Callers that must drop a raw key before the merge go through [`update_config_with_root`].
async fn save_config_root_locked(
    guard: ConfigWriteGuard,
    slot: &Path,
    dest: BoundDest,
    config: &Config,
    ...
    F: FnOnce(&mut TomlValue, &mut Config),
{
    ...
    let path = user_config_path();
    let dest = bind_user_config_dest(&path)?;
    let mut root: TomlValue = crate::config::load_config_file(dest.as_path())?;
    let mut cfg = load_config_from_toml(&root);
    f(&mut root, &mut cfg);
    save_config_root_locked(guard, &path, dest, &cfg, root).await
}
#[cfg(test)]
