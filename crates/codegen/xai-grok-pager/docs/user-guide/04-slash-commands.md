...
Toggle multiline input. When it's on, `Enter` inserts a newline and `Shift+Enter` (or `Alt+Enter`) sends the message. Mid-turn, a bare `Enter` on an empty composer still force-sends the top queued follow-up. Alias: `/ml`. With multiline off, the composer footer shows the newline chord once the draft is non-empty. On the agent screen, `Alt+M` opens the model picker; use this command or `/settings` to toggle multiline. The Agent Dashboard still uses `Ctrl+M` for multiline on the dispatch / peek composer.
