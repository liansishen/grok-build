...
- **The payload is the last one Grok sent.** A timer run re-runs your script with the payload from the last state change, so session numbers — cost, context, and `model_usage` — are as of that change, not of the fire. `process_model_usage` is refreshed from the current agent process after accounted usage arrives.
...
| `process_model_usage` | Usage accumulated since the current agent process started. It is independent of restored session history; an empty map means the current process has not recorded a call yet. Older agents omit this field. |
| `process_model_usage.<model_id>.{input_tokens,output_tokens,reasoning_tokens,total_tokens,cache_creation_input_tokens,cache_read_input_tokens,model_calls,api_duration_ms}` | Direct current-process totals for that model. The token fields use the same uncached-input and cache-bucket semantics as `model_usage`. |
...
| `process_model_usage.<model_id>.{cost_usd,cost_usd_ticks,cost_is_partial}` | Current-process cost when known. Partial or missing cost is not a zero-cost claim. |
| `model_usage.grok-4.5.{input_tokens,output_tokens,reasoning_tokens,total_tokens,cache_creation_input_tokens,cache_read_input_tokens,model_calls,api_duration_ms,cost_usd,cost_usd_ticks,cost_is_partial}` | The wire fixture's concrete model row; scripts should use the model id as a map key rather than hard-code this example. |
| `quota` | CPA quota snapshot for the current model. Omitted when no management quota is available |
| `process_model_usage.grok-4.5.{input_tokens,output_tokens,reasoning_tokens,total_tokens,cache_creation_input_tokens,cache_read_input_tokens,model_calls,api_duration_ms,cost_usd,cost_usd_ticks,cost_is_partial}` | The wire fixture's concrete current-process model row; scripts should use the model id as a map key rather than hard-code this example. |
...
| `quota.accounts` | The list of quota records; each record contains the fields described above. |
