...
fn clipboard_findings(
        ...
        && !facts.clipboard.wrap_sink
    {
        findings.push(manual_finding(
            ...
            t("vscode_non_ascii_msg"),
            t("vscode_non_ascii_note"),
        ));
    }
        ...
        && !facts.clipboard.wrap_sink
    {
        findings.push(manual_finding(
            ...
            FindingDisposition::Recommendation,
            t("iterm2_permission_msg"),
            t("iterm2_permission_note"),
        ));
    }
    findings
}
...
fn newline_finding(facts: &DiagnosticFacts) -> Option<DiagnosticFinding> {
    ...
    let (message, note) = match newline {
        NewlineFact::Vte { version } => (
            ...
            match version {
                Some(version) => {
                    t_fmt("newline_vte_version_note", &[("version", version)])
                }
                None => t("newline_vte_legacy_note").to_owned(),
            },
        ),
        NewlineFact::XtermJs { terminal } => (
            t("newline_xtermjs_msg"),
            t_fmt(
                "newline_xtermjs_note",
                &[("terminal", &terminal.to_string())],
            ),
        ),
        NewlineFact::NoKittyKeyboardProtocol => (
            t("newline_no_kitty_msg"),
            t("newline_no_kitty_note").to_owned(),
        ),
    };
    Some(manual_finding(
