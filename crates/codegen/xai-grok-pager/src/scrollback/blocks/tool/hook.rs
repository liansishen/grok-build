...
fn render_hook_counts_inline_suffix(
    ...
    match shape {
        ...
        HookCountShape::Labeled => {
            ...
            for (count, label_key, color) in [
                (counts.success, "tool.hooks.label.ok", theme.accent_success),
                (
                    counts.blocked,
                    "tool.hooks.label.blocked",
                    theme.accent_running,
                ),
                (counts.failed, "tool.hooks.label.failed", theme.accent_error),
            ] {
                if count == 0 {
                    continue;
                }
