...
impl VerbGroupKind {
    fn i18n_key(self, running: bool, count: usize) -> &'static str {
        ...
        match (self, tense, plurality) {
            ...
            (VerbGroupKind::Message, "running", "one") => "scrollback.verb_group.message.running_one",
            (VerbGroupKind::Message, "running", _) => "scrollback.verb_group.message.running_many",
...
impl<'e> BucketAccumulator<'e> {
    ...
    fn into_label(self, theme: &Theme) -> VerbGroupHeaderLabel {
        ...
        if self.failed_count > 0 {
            let suffix = xai_grok_i18n::t_fmt("scrollback.verb_group.failed_suffix", &[("count", &self.failed_count.to_string())]);
