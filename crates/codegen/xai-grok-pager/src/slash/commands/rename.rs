...
impl SlashCommand for RenameCommand {
    ...
    fn usage(&self) -> &str {
        xai_grok_i18n::t("slash.rename.usage")
    }
    ...
    fn arg_placeholder(&self) -> Option<&str> {
        Some(xai_grok_i18n::t("slash.rename.arg_placeholder"))
    }
    ...
    fn suggest_args(&self, ctx: &AppCtx, args_query: &str) -> Option<Vec<ArgItem>> {
        ...
        Some(vec![ArgItem {
            ...
            description: xai_grok_i18n::t("slash.rename.current_title").to_string(),
        }])
    }
    ...
    fn run(&self, ctx: &mut CommandExecCtx, args: &str) -> CommandResult {
        ...
        if auto_verb_has_trailing_text(args) || auto_verb_has_trailing_text(&title) {
            return CommandResult::Error(
                xai_grok_i18n::t("slash.rename.auto_no_title").to_string(),
            );
        }
        ...
        if title.chars().count() > MAX_TITLE_SCALARS {
            let max = MAX_TITLE_SCALARS.to_string();
            return CommandResult::Error(xai_grok_i18n::t_fmt(
                "slash.rename.title_too_long",
                &[("max", max.as_str())],
            ));
        }
