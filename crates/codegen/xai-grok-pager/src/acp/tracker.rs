    ...
    pub fn label(&self) -> String {
        match self {
            ...
            Self::Hooks { event_name, count } if *count > 1 => {
                xai_grok_i18n::t_fmt("turn.wait.hooks_plural", &[("count", &count.to_string()), ("event", event_name)])
            }
            Self::Hooks { event_name, .. } => {
                xai_grok_i18n::t_fmt("turn.wait.hook", &[("event", event_name)])
            }
            Self::PromptAck => xai_grok_i18n::t("turn.wait.prompt_ack").to_string(),
        }
    }
...
impl WritingToolCall {
    ...
    pub fn label(&self) -> String {
        ...
        match self.tool_name.as_deref() {
            Some(name) if xai_grok_tools::is_task_tool_id(name) => xai_grok_i18n::t_fmt(
                ...
                || tc.title.starts_with("X search:") =>
        {
            ...
            if !success {
                block = block.with_error(failure_reason(
                    tc,
                    xai_grok_i18n::t("tool.error.web_search_failed"),
                ));
            }
        ...
        acp::ToolKind::Search => {
            ...
            if !success {
                block.error = Some(failure_reason(
                    tc,
                    xai_grok_i18n::t("tool.error.search_failed"),
                ));
            }
        ...
        _ if extract_raw_field(tc, "variant").as_deref() == Some("SearchTool") => {
            ...
            if !success {
                let error = block
                    ...
                    .unwrap_or_else(|| xai_grok_i18n::t("tool.error.search_failed").to_owned());
        ...
        _ => {
                ...
                || name.to_ascii_lowercase().starts_with("skill:")
            {
                let label = name
                    ...
                    .map(|rest| xai_grok_i18n::t_fmt("tool.label.skill_named", &[("name", rest)]))
                    .unwrap_or_else(|| xai_grok_i18n::t("tool.label.skill").into());
...
fn extract_edit_error(tc: &acp::ToolCall) -> String {
    ...
    match content_text(tc) {
        text if text.is_empty() => xai_grok_i18n::t("tool.error.edit_failed").to_owned(),
        text => text,
    }
}
