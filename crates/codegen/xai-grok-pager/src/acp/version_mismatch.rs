...
/// (`⚠` → `!` on legacy ConHost). Present in every locale template so
/// dedupe still works when the rest of the toast is translated.
...
pub(crate) fn version_mismatch_banner(params: &str) -> Option<String> {
    ...
    Some(
        sanitize_toast_message(&xai_grok_i18n::t_fmt(
            "toast.version_mismatch",
