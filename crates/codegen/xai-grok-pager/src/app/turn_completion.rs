...
pub(super) fn note_hook_blocked_turn(
    ...
    match requeued {
        ...
        None => agent.show_toast(t("toast.hook_blocked_queue_paused")),
    }
}
...
fn open_prompt_blocked_card(
        ...
        )
    {
        ...
        // silent.
        agent.show_toast(t("toast.prompt_blocked_held"));
        return;
    }
    ...
    let hook_name = blocked
        .hook_name
        .as_deref()
        .unwrap_or(t("prompt_blocked.a_hook"));
    ...
    // queue context.
    let mut question = t_fmt("prompt_blocked.title", &[("hook", short_hook_name)]);
    if !reason.is_empty() {
        question.push_str("\n\n");
    ...
    if was_combined {
        question.push_str("\n\n");
        question.push_str(t("prompt_blocked.combined_submission"));
    }
    if waiting > 0 {
        let waiting_s = waiting.to_string();
        let waiting_key = if waiting == 1 {
            "prompt_blocked.waiting_one"
        } else {
            "prompt_blocked.waiting_many"
        };
        question.push_str("\n\n");
        question.push_str(&t_fmt(waiting_key, &[("waiting", &waiting_s)]));
    }

    let preview = Some(prompt_text);
    let options = vec![
        QuestionOption {
            label: t("prompt_blocked.option_edit").into(),
            description: t("prompt_blocked.option_edit_description").into(),
            preview: preview.clone(),
            id: None,
        },
        QuestionOption {
            label: t("prompt_blocked.option_resend").into(),
            description: t("prompt_blocked.option_resend_description").into(),
            preview: preview.clone(),
            id: None,
        },
        QuestionOption {
            label: t("prompt_blocked.option_discard").into(),
            description: t("prompt_blocked.option_discard_description").into(),
            preview,
            id: None,
        },
    ];
...
pub(super) fn finalize_child_view_turn(
    ...
    if other_prompt {
        if let Some(event) = event {
            child.push_end_marker_block(event, Vec::new(), ending_prompt_id.clone());
        }
        return true;
    }
    push_turn_terminal_marker(child, event, ending_prompt_id.as_deref());
