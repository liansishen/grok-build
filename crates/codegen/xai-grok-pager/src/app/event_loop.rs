...
fn reconcile_billing_poll_deadline(
    ...
    // Keep the timer while an agent is active so CPA weekly quotas can refresh even when consumer xAI billing is hidden (team / API-key).
...
pub(crate) async fn run(
    ...
    if term_state.relaunched_into_minimal && app.screen_mode.is_minimal() {
        app.screen_mode_switch_hint = Some(xai_grok_i18n::t("conn.switched_minimal"));
    } else if term_state.relaunched_into_fullscreen && !app.screen_mode.is_minimal() {
        app.screen_mode_switch_hint = Some(xai_grok_i18n::t("conn.switched_fullscreen"));
    }
                                    ...
                                    match acp_send(load_req, &acp_tx).await {
                                        Ok(resp) => {
                                            loads.push(AgentLoadOutcome {
                                                ...
                                                memory_mode: effects::parse_session_memory_mode(
                                                    resp.meta.as_ref(),
                                                ),
                                            });
                                        }
...
fn process_effects(
    ...
    while let Some(eff) = effs.next() {
        ...
        let Some(mut eff) = effects::take_coalesced_interjects(eff, &mut effs, tasks, &app.acp_tx)
        else {
            continue;
        };
