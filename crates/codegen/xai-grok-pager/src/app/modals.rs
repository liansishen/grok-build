            ...
            ActiveModal::SessionPicker {
                ...
                let config = PickerConfig {
                    ...
                    action_keys: if chat_mode || focused_is_foreign {
                        ...
                        &[('d', t("modal.session_picker.action_delete"))]
                    },
                    disable_search: false,
                    compact_bottom_bar: false,
                    search_only_on_slash: false,
        ...
        if let Some(ref mut active_modal) = self.active_modal {
            ...
            } = active_modal
            {
                ...
                let session_shortcuts: Vec<Shortcut> = if pending_delete.is_some() {
                    vec![
                        Shortcut {
                            label: t("modal.session_picker.confirm_delete"),
                            clickable: false,
                            id: 0,
                        },
                        Shortcut {
                            label: t("modal.session_picker.cancel_delete"),
                            clickable: false,
                            id: 0,
                        },
                    ]
                    ...
                    let mut shortcuts = vec![Shortcut {
                        label: t("modal.session_picker.nav"),
                        clickable: false,
                        id: 0,
                    }];
                    if !external {
                        shortcuts.push(Shortcut {
                            label: t_or("modal.session_picker.expand", "e expand"),
                            clickable: false,
                            id: 0,
                        });
                    }
                    shortcuts.push(Shortcut {
                        label: t("modal.session_picker.search"),
                        clickable: false,
                        id: 0,
                    });
                    if !chat_mode {
                        shortcuts.push(Shortcut {
                            label: t("modal.session_picker.filter"),
                            clickable: false,
                            id: 0,
                        });
                        if !external {
                            shortcuts.push(Shortcut {
                                label: t("modal.session_picker.delete"),
                                clickable: false,
                                id: 0,
                            });
                        }
