...
/// Chooser when editable, browse row when locked. The team-admin arm ensures
/// team identity alone does not lock an administrator out.
#[test]
...
/// `refresh_open_settings_modals` walks into `ResetSettingsConfirm.settings_state` so a `set_X` (or
/// rollback) running while the confirm dialog is open keeps the boxed snapshot fresh. Without this, Cancel would restore a stale snapshot.
#[test]
