...
fn privacy_banner_should_show_respects_gates() {
    ...
    // The banner is gated on the server capability, so an unknown or non-admin
    // team principal gets no banner while a confirmed admin does.
    app.is_team_principal = true;
    app.can_administer_team = Some(false);
    assert!(!app.privacy_banner_should_show(), "non-admin team principal");
    app.can_administer_team = None;
    assert!(!app.privacy_banner_should_show(), "team capability unknown");
    app.can_administer_team = Some(true);
    app.is_team_principal = false;
