        ...
        let persist_canonical: Option<&'static str> = match (in_plan, in_auto, in_yolo) {
            ...
            (true, false, false) => {
                ...
                if auto_gate {
                    ...
                    agent.show_mode_switch_banner(xai_grok_i18n::t("mode.name.normal"));
...
fn mode_choices(
    ...
    choices.push((
        ...
        xai_grok_i18n::t("mode.name.always_approve").to_string(),
    ));
    choices
}
...
fn builtin_mode_choices(auto_gate: bool) -> Vec<(ModeChoice, String)> {
    let mut choices = vec![
        (ModeChoice::Session(SessionMode::Default), xai_grok_i18n::t("mode.name.normal").to_string()),
        (ModeChoice::Session(SessionMode::Plan), xai_grok_i18n::t("mode.name.plan").to_string()),
    ];
    if auto_gate {
        choices.push((ModeChoice::Auto, xai_grok_i18n::t("mode.name.auto").to_string()));
    }
    choices
}
