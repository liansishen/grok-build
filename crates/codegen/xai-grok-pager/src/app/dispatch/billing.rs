...
pub(super) fn handle_credit_limit_recheck_complete(
    ...
    if tier_changed && !user_moved_on {
        if let Some(prompt) = agent.credit_limit_stashed_prompt.take() {
            let Some(tier_name) = app.subscription_tier.as_deref() else {
                unreachable!("tier_changed requires a concrete subscription tier")
            };
            agent.scrollback.push_block(RenderBlock::system(t_fmt(
