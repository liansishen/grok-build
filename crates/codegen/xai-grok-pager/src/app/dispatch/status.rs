...
pub(super) fn dispatch_share_session(app: &mut AppView) -> Vec<Effect> {
    app.show_toast(xai_grok_i18n::t("toast.session_sharing_temporarily_disabled"));
    vec![]
}
...
/// keeps the scrollback flow: session token/cost, then CPA + consumer credits.
...
pub(super) fn handle_coding_data_sharing_failed(
    ...
    let reason = if error.contains(WKE_TEAM_MEMBER_MISSING_ACL) {
        xai_grok_i18n::t("status.coding_data_sharing_team_admin").to_string()
    } else {
