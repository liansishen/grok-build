    ...
    let (session_id, minimal_request_id) = {
        ...
        let Some(session_id) = agent.session.session_id.clone() else {
            if minimal {
                ...
                agent.show_toast(t("toast.no_active_session"));
            }
            return vec![];
        };
...
pub(super) fn dispatch_send_recap(app: &mut AppView, auto: bool) -> Vec<Effect> {
    ...
    if !app.session_recap_available {
        if !auto {
            agent.show_toast(t("toast.recap_disabled"));
        }
        return vec![];
    }
    ...
    if let Some(agent) = app.agents.get_mut(&agent_id) {
        match result {
            ...
            Err(error) => {
                agent
                    .scrollback
                    .push_block(crate::scrollback::block::RenderBlock::system(t_fmt("sys.memory_save_failed", &[("error", &error)])));
            }
        }
    }
    vec![]
