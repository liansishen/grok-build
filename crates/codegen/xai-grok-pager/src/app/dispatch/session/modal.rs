...
pub(in crate::app::dispatch) fn dispatch_sessions_confirm_close(
    ...
    if app.agents.len() == 1 {
        app.show_toast(xai_grok_i18n::t("toast.cannot_close_only_session"));
        return vec![];
    }
...
pub(in crate::app::dispatch) fn dispatch_rename_session(
    ...
    if title.is_empty() {
        agent.scrollback.push_block(RenderBlock::system(
            xai_grok_i18n::t("slash.rename.blank_title").to_string(),
        ));
        return vec![];
    }
    ...
    if kind == xai_grok_shell::session::unified_list::SessionKind::Chat {
        agent
            ...
            .push_block(crate::scrollback::block::RenderBlock::system(
                xai_grok_i18n::t("slash.rename.chat_no_auto_title").to_string(),
            ));
        return vec![];
    }
