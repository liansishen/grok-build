...
pub(in crate::app::dispatch) fn open_delete_current_session_question(
    ...
    {
        xai_grok_i18n::t("delete.option.delete.description_dashboard")
    } else {
        xai_grok_i18n::t("delete.option.delete.description")
    };
    let Some(agent) = app.agents.get_mut(&id) else {
        return vec![];
    };
    ...
    let question = Question {
        question: xai_grok_i18n::t("delete.question").into(),
        id: None,
        options: vec![
            QuestionOption {
                label: xai_grok_i18n::t("delete.option.delete.label").into(),
                description: delete_description.into(),
                preview: None,
                id: None,
            },
            QuestionOption {
                label: xai_grok_i18n::t("delete.option.cancel.label").into(),
                description: xai_grok_i18n::t("delete.option.cancel.description").into(),
                preview: None,
                id: None,
            },
        ],
...
pub(in crate::app::dispatch) fn dispatch_trust_folder(app: &mut AppView) -> Vec<Effect> {
    ...
    match trust_gate_outcome(outcome.resolution(), agent_location(app)) {
        ...
        TrustGateOutcome::FinishSessionLocal => {
            app.show_toast(xai_grok_i18n::t("toast.folder_trust_session_only"));
            finish_trust(app)
        }
...
pub(in crate::app::dispatch) fn handle_session_failed(
    ...
    let msg = match stuck_step {
        ...
        Some(_) | None => xai_grok_i18n::t_fmt("session.create_failed", &[("error", &error)]),
    };
    let is_orphan = app
        .agents
        .get(&agent_id)
...
pub(in crate::app::dispatch) fn handle_worktree_session_failed(
    ...
    let reason = match stuck_step {
        Some(step) if timed_out => crate::app::effects::timed_out_while(step),
        Some(_) | None => xai_grok_i18n::t_fmt("worktree.create_failed", &[("error", &error)]),
    };
