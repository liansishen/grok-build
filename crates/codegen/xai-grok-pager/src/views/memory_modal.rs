...
fn build_shortcuts(state: &MemoryModalState) -> Vec<Shortcut<'static>> {
    ...
    match &state.mode {
        MemoryModalMode::Browse if !state.memory_enabled => {
            ...
            if state.can_enable() {
                shortcuts.push(plain(xai_grok_i18n::t("memory.footer.toggle_on")));
            }
            shortcuts.push(plain(if state.fullscreen { xai_grok_i18n::t("memory.footer.normal") } else { xai_grok_i18n::t("memory.footer.fullscreen") }));
            shortcuts.push(plain(xai_grok_i18n::t("extensions.footer.esc_close")));
            shortcuts
        }
        MemoryModalMode::Browse if !state.has_notes() => {
            vec![
                plain(xai_grok_i18n::t("memory.footer.toggle_off")),
                plain(if state.fullscreen { xai_grok_i18n::t("memory.footer.normal") } else { xai_grok_i18n::t("memory.footer.fullscreen") }),
