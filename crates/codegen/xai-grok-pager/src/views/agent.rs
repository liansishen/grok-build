...
pub(crate) fn build_hints(
    ...
    let mut hints = match active_pane {
        ...
        ActivePane::Dock => {
            ...
            hints.push(HintItem::new(
                crate::key!('h'),
                if show_done { xai_grok_i18n::t("hint.hide_done") } else { xai_grok_i18n::t("hint.show_done") },
            ));
        ...
        ActivePane::Queue => match queue_mutation {
            QueueMutation::ReadOnly => vec![HintItem::new(crate::key!('y'), xai_grok_i18n::t("hint.copy"))],
            QueueMutation::PerRowKind => {
                let mut hints = vec![
                    ...
                    HintItem::paired(crate::key!('J'), crate::key!('K'), xai_grok_i18n::t("hint.reorder")),
        ...
        ActivePane::Prompt => {
            ...
            if !multiline_mode && prompt.can_send() {
                hints.push(if prefer_alt_enter_newline {
                    HintItem::new(newline_key, xai_grok_i18n::t("hint.newline"))
                } else {
                    HintItem::paired(newline_key, crate::key!(Enter, ALT), xai_grok_i18n::t("hint.newline"))
                });
            }
            ...
            if surface == ViewSurface::Root {
                hints.push(HintItem::new(crate::key!(BackTab), xai_grok_i18n::t("hint.mode")));
            }
