...
impl UsageInfoTab {
    ...
    pub fn label(self) -> &'static str {
        match self {
            UsageInfoTab::ContextUsage => xai_grok_i18n::t("usage.modal.tab.context"),
            UsageInfoTab::UsageLimit => xai_grok_i18n::t("usage.modal.tab.limit"),
            UsageInfoTab::SessionInfo => xai_grok_i18n::t("usage.modal.tab.session"),
        }
    }
...
pub fn render_usage_modal(
    ...
    let mut shortcuts: Vec<Shortcut> = vec![
        Shortcut {
            label: xai_grok_i18n::t("usage.modal.footer.tab_switch"),
            clickable: false,
            id: 0,
        },
        Shortcut {
            label: xai_grok_i18n::t("usage.modal.footer.scroll"),
            clickable: false,
            id: 0,
        },
    ];
    if state.ctx.session_id.is_some() {
        shortcuts.push(Shortcut {
            label: xai_grok_i18n::t("usage.modal.footer.copy_session_id"),
            clickable: true,
    ...
    shortcuts.push(Shortcut {
        label: xai_grok_i18n::t("usage.modal.footer.esc_close"),
        clickable: false,
        id: 0,
    });
...
fn context_tab_lines(state: &UsageInfoModalState, theme: &Theme, width: u16) -> Vec<Line<'static>> {
    if let Some(error) = &state.context_error {
        return vec![muted_line(
            theme,
            xai_grok_i18n::t_fmt(
                "usage.modal.context_load_failed",
                &[("error", error.as_str())],
            ),
        )];
    }
    ...
    if state.ctx.session_id.is_none() {
        return vec![muted_line(
            theme,
            xai_grok_i18n::t("usage.modal.no_active_session"),
        )];
    }
    vec![muted_line(
        theme,
        xai_grok_i18n::t("usage.modal.loading_context"),
    )]
}
...
fn usage_limit_lines(
    ...
    if state.ctx.chat_kind {
        ...
        lines.push(muted_line(
            theme,
            xai_grok_i18n::t("usage.modal.team_managed"),
        ));
    } else if let Some(url) = &state.ctx.billing_redirect_url {
        lines.push(plain(
            theme,
            xai_grok_i18n::t_fmt("usage.modal.check_online", &[("url", url.as_str())]),
        ));
        ...
        lines.push(muted_line(
            theme,
            xai_grok_i18n::t("usage.modal.loading_usage"),
        ));
    } else {
        lines.push(muted_line(
            theme,
            xai_grok_i18n::t("usage.billing_unavailable"),
        ));
    }
    ...
    if let Some(usage_text) = &state.session_usage_text {
        ...
        lines.push(muted_line(
            theme,
            xai_grok_i18n::t("usage.modal.loading_session_usage"),
        ));
    }
    lines
}
...
fn allowance_lines(
    ...
    if let Some(reset) = &bal.period_end_display {
        lines.push(muted_line(
            theme,
            xai_grok_i18n::t_fmt("usage.modal.resets", &[("reset", reset.as_str())]),
        ));
    }
    ...
    if let Some(prepaid) = bal.prepaid_balance_cents.map(i64::abs).filter(|c| *c > 0) {
        ...
        lines.push(plain(
            theme,
            xai_grok_i18n::t_fmt("usage.modal.credits", &[("amount", amount.as_str())]),
        ));
    }
    ...
    if bal.pay_as_you_go {
        ...
        lines.push(Line::styled(
            xai_grok_i18n::t("usage.modal.payg_enabled"),
            header_style(theme),
        ));
...
fn session_info_content(state: &UsageInfoModalState, theme: &Theme) -> TabContent {
    if let Some(error) = &state.session_error {
        return TabContent::from_lines(vec![muted_line(
            theme,
            xai_grok_i18n::t_fmt(
                "usage.modal.session_load_failed",
                &[("error", error.as_str())],
            ),
        )]);
    }
    let Some(fields) = state.session_fields.as_ref().filter(|f| !f.is_empty()) else {
        ...
        return TabContent::from_lines(vec![muted_line(
            theme,
            xai_grok_i18n::t("usage.modal.loading_session_info"),
        )]);
    };
    ...
    let mut lines = vec![Line::from(vec![
        Span::styled(
            xai_grok_i18n::t("usage.modal.session_info_header"),
            header_style(theme),
        ),
        Span::styled(
            xai_grok_i18n::t("usage.modal.click_values_to_copy"),
            Style::default().fg(theme.gray_dim),
        ),
    ])];
