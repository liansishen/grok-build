...
pub fn render_goal_detail(
    ...
    let (pct, budget_display) = if let Some(budget) = goal.token_budget.filter(|&b| b > 0) {
        ...
        (
            0.0,
            t_fmt("goal.tokens_count", &[("count", &tokens_str)]),
        )
    };
    let has_budget = goal.token_budget.is_some_and(|b| b > 0);
...
mod tests {
    ...
    fn humanize_event_timestamp_relative_and_fallback() {
        ...
        // A fresh RFC3339 stamp → the localized immediate-time label; ~2h
        // ago → the localized coarse hour bucket wrapped by goal.time_ago.
        let now = chrono::Utc::now().to_rfc3339();
        let just_now = xai_grok_i18n::t_or("time_ago.just_now", "just now");
        assert_eq!(humanize_event_timestamp(&now), just_now);
        let past = (chrono::Utc::now() - chrono::Duration::hours(2)).to_rfc3339();
        let hours = xai_grok_i18n::t_or("time_ago.hours", "{count}h").replace("{count}", "2");
        assert_eq!(
            humanize_event_timestamp(&past),
            t_fmt("goal.time_ago", &[("time", &hours)])
        );
    }
    ...
    fn recent_history_renders_humanized_event_and_relative_time() {
        ...
        let minutes = xai_grok_i18n::t_or("time_ago.minutes", "{count}m")
            .replace("{count}", "2");
        let relative_time = t_fmt("goal.time_ago", &[("time", &minutes)]);
        assert!(
            text.contains(&relative_time),
            "relative time, got:\n{text}"
        );
        assert!(
