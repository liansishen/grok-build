...
fn workflows_empty_placeholder() -> String {
    t_or("extensions.empty.workflows", WORKFLOWS_EMPTY_PLACEHOLDER).to_string()
}
...
pub(super) fn build_workflows_picker_rows(
    ...
    let workflows = match data {
        ...
        TabDataState::Error(msg) => {
            return vec![WorkflowRow::notice(t_fmt("extensions.error", &[("msg", msg)]))]
        }
