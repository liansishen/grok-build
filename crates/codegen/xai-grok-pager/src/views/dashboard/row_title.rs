...
impl RowTitle<'_> {
    ...
    fn fit_chips(&self, budget: usize, available: usize) -> Line<'static> {
        ...
        if total > 0 {
            if let Some(chips) = choose(Line::raw(xai_grok_i18n::t_fmt(
                "dashboard.badge.background_count",
                &[("total", &total.to_string())],
            ))) {
                return chips;
            }
            if let Some(chips) = choose(Line::raw(xai_grok_i18n::t("dashboard.badge.background"))) {
                return chips;
            }
        }
        fallback
...
const CHIP_SEP: &str = " · ";
/// Separator plus the translated failure suffix; rebuilt per call because the label is localized.
fn failed_label() -> String {
    format!(" · {}", xai_grok_i18n::t("dashboard.badge.failed"))
}
...
fn paint_failed(buf: &mut Buffer, x: u16, y: u16, theme: &Theme, bg: Color) {
    ...
    buf.set_string(
        cx,
        y,
        xai_grok_i18n::t("dashboard.badge.failed"),
