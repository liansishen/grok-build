...
pub(super) fn render_header(
    ...
    let mut choose_hint = hint_line(
        Span::styled(
            xai_grok_i18n::t("dashboard.location.choose").to_string(),
            dim,
        ),
        chord_hint(
            theme,
...
pub(super) fn render_actions_row(
    ...
    let new_agent_label = if worktree_armed {
        xai_grok_i18n::t("dashboard.new_worktree_action").to_string()
    } else {
        xai_grok_i18n::t("dashboard.new_agent_action").to_string()
    };
    let new_agent_w = (UnicodeWidthStr::width(new_agent_label.as_str()) as u16).min(area.width);
    ...
    let worktree_label = if worktree_armed {
        xai_grok_i18n::t("dashboard.worktree_disable").to_string()
    } else {
        xai_grok_i18n::t("dashboard.worktree").to_string()
    };
    ...
    if workspace_dashboard_enabled && worktree_rect.is_some() {
        ...
        let open_previous = hint_line(
            Span::styled(
                xai_grok_i18n::t("dashboard.open_previous").to_string(),
                bg.fg(button_fg(
