...
/// Dim `hide: /announcements hide` hint right of the title row; the slash command itself is not translated.
fn hide_cta() -> &'static str {
    xai_grok_i18n::t("announcements.hide_cta")
}

/// Clickable hide button, far right of the title row.
fn hide_button() -> &'static str {
    xai_grok_i18n::t("announcements.hide_button")
}
...
mod tests {
    ...
    /// Both hide affordances are catalog copy: the pseudo-locale markers prove the button and the CTA reach it.
    #[test]
    fn hide_affordances_come_from_the_catalog() {
        let anns = [ann(Some("critical"), Some("Scheduled maintenance"))];
        let area = Rect::new(0, 0, 120, 2);
        let text = xai_grok_i18n::with_pseudo_locale(|| {
            let mut buf = Buffer::empty(area);
            render_banner(area, &mut buf, &anns, &no_hidden(), false, false, true);
            (area.y..area.y + area.height)
                .map(|y| buf_row(&buf, area, y))
                .collect::<Vec<_>>()
                .join("\n")
        });
        assert!(
            text.contains("⟦announcements.hide_button⟧"),
            "hide button must come from the catalog: {text:?}"
        );
        assert!(
            text.contains("⟦announcements.hide_cta⟧"),
            "hide CTA must come from the catalog: {text:?}"
        );
    }
}
