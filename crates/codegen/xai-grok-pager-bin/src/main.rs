    ...
    fn notice(&self) -> Option<String> {
        match self {
            ...
            Self::Clamped {
                ...
                Some(t_fmt(
                    "cli.runtime.worker_threads_clamped",
                    &[
            ...
            Self::Ignored { value, .. } => {
                ...
                Some(t_fmt(
                    "cli.runtime.worker_threads_invalid",
                    &[("env", GROK_WORKER_THREADS_ENV), ("value", value.as_str())],
                ))
            }
        }
    }
...
fn main() {
    ...
    // Apply locale before early CLI dispatch and startup diagnostics.
        ...
        xai_tty_utils::runtime::build_with_blocking_pool(&mut builder).unwrap_or_else(|e| {
            ...
            eprintln!(
                ...
                t_fmt(
                    ...
                    &[("workers", workers.as_str()), ("error", error.as_str())],
                )
            );
