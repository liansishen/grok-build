...
fn describe_access(access: &AccessKind) -> String {
    match access {
        ...
        AccessKind::AgentMessage { subagent_id } => xai_grok_i18n::t_fmt(
            "permission.description.agent_message",
            &[("subagent_id", subagent_id.as_str())],
        ),
        AccessKind::Tool(name) => {
            xai_grok_i18n::t_fmt("permission.description.tool", &[("name", name.as_str())])
        }
    }
}
