...
fn duration_line(description: Option<&str>, duration_secs: f64) -> String {
    let duration_label = xai_grok_i18n::t_or("task_completion.duration_label", "Duration");
    match description {
        Some(desc) => format!(
            "{}: {desc} | {duration_label}: {duration_secs:.1}s",
            xai_grok_i18n::t_or("task_completion.description_label", "Description")
        ),
        None => format!("{duration_label}: {duration_secs:.1}s"),
    }
}
...
pub fn format_bash_completion(
    ...
    let mut msg = format!(
        "{} \"{}\" completed ({}).\n{}\n{}",
        xai_grok_i18n::t_or("task_completion.background_prefix", "Background task"),
        task.task_id,
        status_str,
        identity,
        notice,
...
pub fn format_monitor_completion(task: &TaskSnapshot, task_output_name: Option<&str>) -> String {
    ...
    format!(
        "{} \"{}\" ended: [monitor ended: {}].\n{}\nUse {}(\"{}\") for full output.\n{}",
        xai_grok_i18n::t_or("task_completion.monitor_prefix", "Monitor"),
        task.task_id,
        reason,
        identity,
        tool,
