...
fn committed_edit_keeps_diff_line_backgrounds() {
    ...
    // The fork's transparent canvas clears the diff row bands (see `with_transparent_canvas`);
    // this regression pins the banded palette so the band assertions below stay meaningful.
    theme_cache::set_transparent_bg(false);
